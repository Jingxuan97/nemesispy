...
# Elcipse spectra that we want to fit (from Stevenson et al. 2018)
elipse = np.array(
    [0.000367, 0.000431, 0.000414, 0.000482, 0.00046 , 0.000473,
    0.000353, 0.000313, 0.00032 , 0.000394, 0.000439, 0.000458,
    0.000595, 0.000614, 0.000732, 0.003231, 0.003827])
error = np.array(
    [4.5e-05, 3.9e-05, 3.8e-05, 3.6e-05, 3.7e-05, 3.3e-05, 3.4e-05,
...
# Initial log10 gas volume mixing ratios
H2O_init = -8
CO2_init = -8
CO_init = -8
CH4_init = -8
VMR_model_init = gen_vmr(NLAYER,10**H2O_init,10**CO2_init,10**CO_init,
    10**CH4_init)
# Initial temperature-pressure profile
k_IR0 = 10**-3
gamma0 = 10**-2
f0 = 0.1
T_int0 = 100
...
FM = ForwardModel()
FM.set_planet_model(
    M_plt=M_plt,R_plt=R_plt,gas_id_list=gas_id,iso_id_list=iso_id,
    NLAYER=NLAYER)
FM.set_opacity_data(
    kta_file_paths=lowres_file_paths, cia_file_path=cia_file_path)
