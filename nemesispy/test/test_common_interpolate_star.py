import unittest
import numpy as np
from nemesispy.common.interpolate_star import interpolate_stellar_spectrum


class TestInterpolateStellarSpectrum(unittest.TestCase):
    ...
    def test_wasp43(self):
        interped_spec = interpolate_stellar_spectrum(
            self.wave_grid, self.wave_data, self.spec_data
            )
        for iwave in range(len(interped_spec)):
            self.assertAlmostEqual(
                (interped_spec[iwave]
                 - self.wasp43_spec[iwave])/interped_spec[iwave], 0
                )
