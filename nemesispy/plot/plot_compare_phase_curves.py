...
retrieved_TP_phase_by_wave_Guillot_fixed_T_int = np.zeros((nphase,nwave))
retrieved_TP_wave_by_phase_Guillot_fixed_T_int = np.zeros((nwave,nphase))
T_profiles_Guillot_fixed_T_int \
    = np.loadtxt('AAbest_fit_Guillot_fixed_T_int.txt',ndmin=2,delimiter=',')
...
for iphase, phase in enumerate(phase_grid):
    one_phase =  FM.calc_disc_spectrum(phase=phase, nmu=nmu, P_model=P_range,
        global_model_P_grid=pv, global_T_model=retrieved_T_map_Guillot_fixed_T_int,
        global_VMR_model=vmrmap_mod, mod_lon=xlon, mod_lat=xlat,
        solspec=wasp43_spec)
    retrieved_TP_phase_by_wave_Guillot_fixed_T_int[iphase,:] = one_phase
for iwave in range(len(wave_grid)):
    for iphase in range(len(phase_grid)):
        retrieved_TP_wave_by_phase_Guillot_fixed_T_int[iwave,iphase] \
            = retrieved_TP_phase_by_wave_Guillot_fixed_T_int[iphase,iwave]
...
iy = 0
for iphase,phase in enumerate(phase_grid):
    axs[ix,iy].plot(wave_grid, retrieved_TP_phase_by_wave_Guillot_fixed_T_int[iphase,:]*1e3,
...
for iwave,wave in enumerate(wave_grid[::-1]):
    ...
    axs[ix,iy].plot(phase_grid, retrieved_TP_wave_by_phase_Guillot_fixed_T_int[16-iwave,:]*1e3,
