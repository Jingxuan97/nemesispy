...
line_spec, = ax[0].plot(wave_grid,spec,marker='x',markersize=3,
        label='model')
line_clear, =  ax[0].plot(wave_grid,spec,color='k',
        label='pure H2/He')
...
sf = Slider(axf, 'f', 0.0, 2.0, valinit=f0, valstep=0.1)
...
def update(val):
    ...
    clear = FM.calc_disc_spectrum_uniform(
        3,P_model,T_model,VMR_model_init,
        solspec=wasp43_spec)
    spec = FM.calc_disc_spectrum_uniform(
