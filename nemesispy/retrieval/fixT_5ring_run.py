...
grid_longitudes = np.linspace(-175,175,71)
grid_latitudes = np.linspace(5,85,17)
...
def LogLikelihood(cube,ndim,nparams):
    ...
    vmr_grid = gen_vmrmap1(cube[16],cube[17],cube[18],cube[19],
        nlon=len(grid_longitudes), nlat=len(grid_latitudes),
        npress=len(P_range))
    ...
    for iphase, phase in enumerate(phase_grid):
        ...
        one_phase =  FM.calc_disc_spectrum(phase=phase, nmu=nmu, P_model=P_range,
            ...
            global_VMR_model=vmr_grid, mod_lon=grid_longitudes,
            mod_lat=grid_latitudes, solspec=wasp43_spec)
...
n_params = 20
print('start')
folder_name = 'fixT_nmu_{}'.format(nmu)
file_base = folder_name + '/out-'
