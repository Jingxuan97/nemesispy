...
def gen_vmrmap_1d_v2(log_h2o,log_co2,log_co,log_ch4,log_nh3,npress,h2_frac = 0.84):
    """
    Generate a 1D uniform gas abundance map.
    The abundance map is defined on a pressure grid.
    ...
    """
    he_frac = 1 - h2_frac
    vmr_grid = np.ones((npress,7))
    vmr_grid[:,0] *= 10**log_h2o
    vmr_grid[:,1] *= 10**log_co2
    vmr_grid[:,2] *= 10**log_co
    vmr_grid[:,3] *= 10**log_ch4
    vmr_grid[:,4] *= 10**log_nh3
    vmr_grid[:,5] *= he_frac * (1-10**log_h2o-10**log_co2-10**log_co-10**log_ch4-10**log_nh3)
    vmr_grid[:,6] *= h2_frac * (1-10**log_h2o-10**log_co2-10**log_co-10**log_ch4-10**log_nh3)
    return vmr_grid
...
def gen_vmrmap2(log_h2o,log_co2,log_co,log_ch4,log_nh3,nlon,nlat,npress,
    ...
    """
    he_frac = 1 - h2_frac
    vmr_grid = np.ones((nlon,nlat,npress,7))
    vmr_grid[:,:,:,0] *= 10**log_h2o
    vmr_grid[:,:,:,1] *= 10**log_co2
    vmr_grid[:,:,:,2] *= 10**log_co
    vmr_grid[:,:,:,3] *= 10**log_ch4
