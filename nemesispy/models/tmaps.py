...
def tmap_cos_n_flat_line(P_grid, lon_grid, lat_grid, g_plt, T_eq,
    scale, phase_offset,
...
def tmap_cos_n_flat_line_lat(P_grid, lon_grid, lat_grid, g_plt, T_eq,
    scale, phase_offset,
    log_kappa_day, log_gamma_day1, log_gamma_day2, alpha_day, log_beta_day, T_int_day,
    log_kappa_night, log_gamma_night1, log_gamma_night2, alpha_night, log_beta_night, T_int_night,
    n):
    ...
    for ilon,lon in enumerate(lon_grid):
        if lon < bound_west or lon > bound_east:
            for ilat,lat in enumerate(lat_grid):
                tp_out[ilon,ilat,:] = tp_night*np.cos(lat*dtr)**0.25
        else:
            arg = (lon-phase_offset)/scale*dtr
            tp = tp_night + (tp_day-tp_night) * np.cos(arg) ** n
            for ilat,lat in enumerate(lat_grid):
                tp_out[ilon,ilat,:] = tp*np.cos(lat*dtr)**0.25

    return tp_out

def tmap_cos_n_flat_line_lat_n(P_grid, lon_grid, lat_grid, g_plt, T_eq,
    scale, phase_offset,
    log_kappa_day, log_gamma_day1, log_gamma_day2, alpha_day, log_beta_day, T_int_day,
    log_kappa_night, log_gamma_night1, log_gamma_night2, alpha_night, log_beta_night, T_int_night,
    n,
