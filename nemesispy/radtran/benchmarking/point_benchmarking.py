...
P_delH, P_totam, P_pres, P_temp, P_scaling \
    = FM.del_H, FM.U_layer, FM.P_layer, FM.T_layer, FM.scale

"""
point_spectrum_py_comp = FM.test_point_spectrum(U_layer=F_totam,P_layer=F_pres,
                        T_layer=F_temp, VMR_layer=VMR, del_S=F_delH,
                        scale=scaling, solspec=stellar_spec)
"""
point_spectrum_py_comp = FM.test_point_spectrum(U_layer=F_totam,P_layer=P_pres,
                        T_layer=P_temp, VMR_layer=VMR, del_S=P_delH,
                        scale=P_scaling, solspec=stellar_spec)
...
#os.system("python3 /Users/jingxuanyang/Desktop/uptodate/NemesisPy-dist/NemesisPy/Programs/nemesisPY.py < pointspec.nam")
...
axs[0].scatter(wave_grid, end_to_end_spectrum_py, marker='.', color='b',
    linewidth=1, s=10, label='hydro')
axs[0].plot(wave_grid, end_to_end_spectrum_py, color='b', linewidth=0.5)
# Juan's work
#axs[0].scatter(wave_grid, juan_version, marker='.', color='c',
#    linewidth=1, s=10, label='juan_version')
#axs[0].plot(wave_grid, juan_version, color='c', linewidth=0.5)
...
diff_0 = (point_spectrum_fo-point_spectrum_py_comp)/point_spectrum_fo
axs[1].scatter(wave_grid,diff_0,
    marker='x',color='c',s=20, label='test rad comp')
...
#    marker='.',color='c',label='diff (juan)')
