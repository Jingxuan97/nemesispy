...
###
VMR_list = [
    [1e-3,1e-10,1e-10,1e-10],
    [1e-10,1e-3,1e-10,1e-10],
    [1e-10,1e-10,1e-3,1e-10],
    [1e-10,1e-10,1e-10,1e-3]
]
VMR_type_list = ['H2O','CO2','CO','CH4']
...
for index, vmr in enumerate(VMR_list):
    ...
    ## Upper panel
    # Fortran plot
    axs[index,0].plot(wave_grid,spec_fo,color='k',linewidth=0.5,linestyle='-',
        marker='x',markersize=2,label='Fortran')

    # Python model plot
    axs[index,0].plot(wave_grid, spec_py,color='b',linewidth=0.5,linestyle='--',
        marker='x',markersize=2,label='Python')

    # Mark the type of TP profile
    axs[index,0].text(1.3,2.5e-3,"{}".format(VMR_type_list[index]))

    ## Upper panel format
    axs[index,0].ticklabel_format(axis="y", style="sci", scilimits=(0,0))
    # axs[index,0].legend(loc='upper left')
    axs[index,0].grid()
