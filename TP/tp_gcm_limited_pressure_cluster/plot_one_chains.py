...
a = pymultinest.Analyzer(n_params=n_params,outputfiles_basename='chains/{}_{}-'.format(ilon,ilat))
...
figure=corner(samples,
              ...
              plot_contours='True',
              truths=None)
figure.savefig("TP_triangle_{}_{}.pdf".format(ilon,ilat))
plt.close()
...
plt.tight_layout()
plt.savefig('Spread_TP_{}_{}.pdf'.format(ilon,ilat), format='pdf')
plt.show()
# plt.close()
...
plt.gca().invert_yaxis()
plt.tight_layout()
plt.savefig('Tmedian_{}_{}.pdf'.format(ilon,ilat),format='pdf')
plt.show()
# plt.close()
